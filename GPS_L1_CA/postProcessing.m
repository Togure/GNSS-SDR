...
for chan = 1:1:5
    E(chan) = mean(sqrt(trackResults(chan).I_E.^2 + trackResults(chan).Q_E.^2));
    P(chan) = mean(sqrt(trackResults(chan).I_P.^2 + trackResults(chan).Q_P.^2));
    L(chan) = mean(sqrt(trackResults(chan).I_L.^2 + trackResults(chan).Q_L.^2));
end
figure,plot([-0.5,0,0.5],[E;P;L]);
